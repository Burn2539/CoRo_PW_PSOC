...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_USB_UART) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        USB_UART_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != USB_UART_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = USB_UART_GetRxBufferSize();
                CyDelay(USB_UART_BYTE2BYTE_TIME_OUT);
            }
            while(USB_UART_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  USB_UART_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(USB_UART_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(USB_UART_GetRxBufferSize() > 0u)
    {
        tempCount = USB_UART_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = USB_UART_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional USB_UART_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != USB_UART_PACKET_EOP)
            {
                CyDelay(USB_UART_WAIT_EOP_DELAY);
            }
        }
